...
本仓库的 v0.1.0rc1 只发布最小部署骨架、工作区规则和非秘密配置说明。应用源码、配置 schema、运行行为和应用级 Release Note 归属于 [loong-dashboard](https://github.com/MrZ20-System/loong-dashboard)，不在此仓库重复维护。

实际运行数据和 secrets 永不随版本发布，包括本机 `system.yaml`、`system.yaml*.bak` 迁移备份、`settings.json`、`domains/`、`prompts/`、`.loong/`、`.worktrees/` 以及任何 GitHub、Agent 或部署凭据。新机器应从应用仓库的 canonical 示例生成本地配置，再由部署环境安全注入秘密值。
...
| system.yaml、system.yaml*.bak、settings.json | 本机部署数据 | 实例配置、迁移备份、账户验证状态和调度设置；不提交到 system-home |
